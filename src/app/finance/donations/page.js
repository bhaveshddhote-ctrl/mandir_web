...
export default function DonationsPage() {
  ...
  const fetchDonations = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDonations(data.filter(e => e.type === 'in'));
      }
    } catch (err) {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        showToast('✅ Donation entry added successfully!');
      } else {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Is donation entry ko delete karna chahte hain?')) return;
    try {
      ...
      <DashboardLayout>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
            <div>
              ...
              <p style={{ color: '#64748B', fontSize: '0.9rem' }}>
                Guru Gorakhnath Math mein prapt sabhi daan ka vivaran
              </p>
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={fetchDonations} style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px' }}>
            <div style={{ ...cardStyle, borderLeft: '4px solid #16A34A' }}>
              <p style={{ color: '#64748B', fontSize: '0.8rem', marginBottom: '4px' }}>Total Donations</p>
              <h3 style={{ fontSize: '1.6rem', fontWeight: '800', color: '#16A34A' }}>₹{totalAmount.toLocaleString('en-IN')}</h3>
            </div>
            <div style={{ ...cardStyle, borderLeft: '4px solid #D4601A' }}>
              <p style={{ color: '#64748B', fontSize: '0.8rem', marginBottom: '4px' }}>Total Entries</p>
              <h3 style={{ fontSize: '1.6rem', fontWeight: '800', color: '#D4601A' }}>{donations.length}</h3>
            </div>
          </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.88rem' }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #F1F5F9' }}>
                    ...
                    <th style={{ padding: '12px 16px', textAlign: 'left', color: '#94A3B8', fontWeight: 600, fontSize: '0.75rem', textTransform: 'uppercase' }}>Donor Name</th>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan={5} style={{ textAlign: 'center', padding: '48px', color: '#94A3B8' }}>
                      ...
                      <div style={{ fontWeight: 600, marginBottom: '4px' }}>Koi donation entry nahi mili</div>
                      <div style={{ fontSize: '0.82rem' }}>Upar "Add Donation" button se entry karo</div>
                    </td></tr>
                  ) : (
                    filtered.map((item, i) => (
                      ...
                      >
                        ...
                        <td style={{ padding: '14px 16px', fontWeight: 600, color: '#1E293B' }}>{item.name || <span style={{ color: '#CBD5E1' }}>Anonymous</span>}</td>
                        <td style={{ padding: '14px 16px', color: '#475569' }}>{item.desc}</td>
                        <td style={{ padding: '14px 16px', textAlign: 'right', fontWeight: 700, color: '#16A34A' }}>+₹{item.amount?.toLocaleString('en-IN')}</td>
          ...
          <div style={{
            ...
            <div style={{ background: '#fff', borderRadius: '20px', padding: '32px', width: '100%', maxWidth: '460px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <h3 style={{ fontSize: '1.2rem', fontWeight: '800', color: '#1E293B' }}>➕ New Donation Entry</h3>
              ...
              <form onSubmit={handleAdd} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#64748B', marginBottom: '6px', textTransform: 'uppercase' }}>Donor Name (Optional)</label>
                  <input type="text" placeholder="e.g. Ram Prakash Gupta" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
                    style={{ width: '100%', padding: '10px 14px', border: '1px solid #E2E8F0', borderRadius: '10px', outline: 'none', fontSize: '0.9rem' }} />
                </div>
                <div>
                  ...
                  <input type="text" placeholder="e.g. Guru Purnima Daan, Bhandara Contribution" value={form.desc} onChange={e => setForm({ ...form, desc: e.target.value })} required
                    style={{ width: '100%', padding: '10px 14px', border: '1px solid #E2E8F0', borderRadius: '10px', outline: 'none', fontSize: '0.9rem' }} />
                </div>
                <div>
        ...
        {toast && (
          <div style={{
            ...
            borderLeft: '4px solid #16A34A', display: 'flex', alignItems: 'center', gap: '8px'
          }}>
            <CheckCircle size={16} color="#16A34A" /> {toast}
          </div>
        )}
